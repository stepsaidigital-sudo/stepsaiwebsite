...
# We replace V5 hero with V6, which drastically tightens the vertical spacing
# to ensure the chat widget is visible above the fold on standard screens.

CUSTOM_HOME_HERO_V6 = """
CUSTOM_HOME_HERO = \"\"\"
  <!-- S01 Landing Hero - Centered with Dynamic Chat Widget (V6 Tight Spacing) -->
  <section class="hero-v2 centered-hero" style="position: relative; padding-top: 120px; padding-bottom: 0px; overflow: hidden; display: flex; flex-direction: column; align-items: center; text-align: center; max-width: 1400px; margin: 0 auto; min-height: 100vh;">
    ...
    <div id="hero-bg-wa" style="position: absolute; left: -120px; top: 15%; transform: translateY(-50%) rotate(-8deg); z-index: 0; opacity: 0; filter: blur(2px); transition: filter 0.5s;" onmouseover="this.style.filter='blur(0)'" onmouseout="this.style.filter='blur(2px)'">
        <div class="phone-mockup" style="transform: scale(0.65);">
    ...
    <div style="position: relative; z-index: 10; max-width: 800px; display: flex; flex-direction: column; align-items: center; width: 100%;">
      ...
      <h1 class="gsap-fade-up" style="font-family: 'Outfit'; font-size: clamp(48px, 7vw, 76px); margin-bottom: 16px; line-height: 1.05; letter-spacing: -0.03em; color: var(--text-primary); font-weight: 700;">It answers.<br>Then it acts.</h1>
      ...
      <div class="hero-actions gsap-fade-up" style="display:flex; gap:12px; flex-wrap: wrap; justify-content: center; align-items: center;">
        <a href="./partners/apply/index.html" style="text-decoration: none;">
           <button class="btn-primary" style="padding: 14px 28px; font-size: 16px; border-radius: 100px; display: flex; align-items: center; gap: 8px;">
              Start Free Trial 
        ...
        <button class="btn-outline" style="padding: 14px 28px; font-size: 16px; border-radius: 100px;">Book an Appointment</button>
      </div>
    ...
    <div id="hero-main-web" class="gsap-fade-up" style="position: relative; z-index: 5; margin-top: 32px; width: 100%; max-width: 380px; opacity: 0; transform: translateY(100px);">
        
        <!-- Real Chat Widget Mockup container -->
        <div style="width: 100%; background: #fbfbfd; border-radius: 20px 20px 0 0; box-shadow: 0 32px 80px rgba(0,0,0,0.15); border: 1px solid var(--border-subtle); border-bottom: none; overflow: hidden; display: flex; flex-direction: column; height: 400px; position: relative;">
           ...
           <div id="chat-screen-home" style="position: absolute; inset: 0; display: flex; flex-direction: column; z-index: 2; background: #fbfbfd; opacity: 1;">
               <div style="background: linear-gradient(180deg, #24252e 0%, #17181e 100%); padding: 20px 16px 24px; position: relative; text-align: center;">
                  <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 16px;">
                     <div style="width: 32px; height: 32px; background: linear-gradient(135deg, #7b2cbf, #5a189a); border-radius: 8px; display: flex; justify-content: center; align-items: center;">
           ...
           <div id="chat-screen-msg" style="position: absolute; inset: 0; display: flex; flex-direction: column; z-index: 1; background: #fff; opacity: 0; pointer-events: none;">
               ...
               <div style="background: #fff; padding: 12px 16px; display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #efefef;">
                  ...
                  <div style="display: flex; align-items: center; gap: 10px; flex: 1; margin-left: 12px;">
                     <div style="width: 36px; height: 36px; background: linear-gradient(135deg, #7b2cbf, #5a189a); border-radius: 50%; display: flex; justify-content: center; align-items: center;">
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        function playIndustryLoop(index) {
            ...
            // 2. Type out query on Home screen
            ...
            // 4. Instant Crossfade to Messages Screen
            ctl.to(screenHome, { opacity: 0, duration: 0.3, ease: "power2.inOut" }, "+=0.4")
            ...
            // 5. Show user bubble pop in
            ctl.to("#msg-u", { opacity: 1, y: 0, duration: 0.4, ease: "back.out(1.2)" }, "+=0.2");
               
            // 6. Show typing indicator
            ctl.to("#msg-b", { opacity: 1, y: 0, duration: 0.4, ease: "power3.out" }, "+=0.3")
               .to("#msg-t", { opacity: 1, duration: 0.2 });
               
            // 7. Swap typing for response
            ctl.call(() => {
                document.getElementById('msg-t').style.display = 'none';
                document.getElementById('msg-r').style.display = 'block';
            }, null, "+=1.5");
            
            // 8. Fade out the chat content gently before looping back to home
            ctl.to(chatContentEl, { opacity: 0, duration: 0.4, ease: "power2.inOut" }, "+=3");
        }
        
        // Start loop 2 seconds after hero intro completes
...
if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + CUSTOM_HOME_HERO_V6 + "\n" + content[end_idx:]
    with open('build_v11_compiler.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Injected V6 Centered Hero with Correct Spacing")
else:
