...
# We replace the V4 hero with V5, which fixes the animation flow:
# Typing happens on the Home Screen FIRST, then it transitions to the Messages screen.

CUSTOM_HOME_HERO_V5 = """
CUSTOM_HOME_HERO = \"\"\"
  <!-- S01 Landing Hero - Centered with Dynamic Chat Widget (V5 Flow) -->
           ...
           <div id="chat-screen-home" style="position: absolute; inset: 0; display: flex; flex-direction: column; z-index: 2; background: #fbfbfd; opacity: 1;">
               ...
               <div style="padding: 0 20px; position: relative; margin-top: -26px;">
                  <div style="background: #fff; border-radius: 20px; padding: 18px 24px; display: flex; align-items: center; justify-content: space-between; box-shadow: 0 8px 24px rgba(0,0,0,0.06); border: 1px solid #efefef;">
                     <div id="home-typewriter" style="color: #888; font-size: 16px; font-weight: 500; flex: 1; text-align: left; white-space: nowrap; overflow: hidden;">Ask me anything</div>
                     <div id="home-send-btn" style="width: 36px; height: 36px; border-radius: 50%; background: #f4f5f7; display: flex; justify-content: center; align-items: center; color: #444; transition: background 0.3s, color 0.3s;">
           ...
           <div id="chat-screen-msg" style="position: absolute; inset: 0; display: flex; flex-direction: column; z-index: 1; background: #fff; opacity: 0; pointer-events: none;">
               ...
               <div style="padding: 16px 20px; background: #fbfbfd; border-top: 1px solid #efefef; display: flex; flex-direction: column; gap: 12px;">
                  <div style="background: #fff; border-radius: 20px; padding: 12px 16px; border: 1px solid #efefef; display: flex; align-items: center;">
                     <div style="flex: 1; font-size: 15px; color: #888; text-align: left;">Ask me anything</div>
  ...
  <style>
    ...
    .msg-user-bubble { background: #625df5; color: #fff; padding: 12px 16px; border-radius: 12px; border-bottom-right-radius: 4px; font-size: 14px; max-width: 240px; font-weight: 500; text-align: left; }
    ...
    .msg-bot-bubble { background: #f4f5f7; color: #111; padding: 16px; border-radius: 12px; border-top-left-radius: 4px; font-size: 14px; max-width: 300px; line-height: 1.5; text-align: left; }
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        function playIndustryLoop(index) {
            ...
            // 4. Instant Crossfade to Messages Screen (No slider!)
...
if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + CUSTOM_HOME_HERO_V5 + "\n" + content[end_idx:]
    with open('build_v11_compiler.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Injected V5 Centered Hero with Correct Animation Flow")
else:
