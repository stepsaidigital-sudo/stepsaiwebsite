...
CUSTOM_HOME_HERO_V9 = """
CUSTOM_HOME_HERO = \"\"\"
  <!-- S01 Landing Hero - Centered with Dynamic Chat Widget (V9 BOLD Premium Gradient Background) -->
  <section class="hero-v2 centered-hero" style="position: relative; padding-top: 120px; padding-bottom: 0px; overflow: hidden; display: flex; flex-direction: column; align-items: center; text-align: center; max-width: 1400px; margin: 0 auto; min-height: 100vh;">
    ...
    <!-- Advanced Animated Mesh Gradient Background - BOLD & PREMIUM -->
    <div class="advanced-mesh-bg" style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; z-index: 0; overflow: hidden; background: #0a0b10;">
        <div class="mesh-orb-1" style="position: absolute; top: -20%; left: -10%; width: 900px; height: 900px; background: radial-gradient(circle, rgba(26,86,219,0.7) 0%, rgba(26,86,219,0) 70%); border-radius: 50%; filter: blur(90px); animation: float1 15s infinite ease-in-out;"></div>
        <div class="mesh-orb-2" style="position: absolute; top: 10%; right: -20%; width: 1000px; height: 1000px; background: radial-gradient(circle, rgba(123,44,191,0.6) 0%, rgba(123,44,191,0) 70%); border-radius: 50%; filter: blur(100px); animation: float2 20s infinite ease-in-out;"></div>
        <div class="mesh-orb-3" style="position: absolute; bottom: -30%; left: 20%; width: 1200px; height: 1200px; background: radial-gradient(circle, rgba(11,158,88,0.4) 0%, rgba(11,158,88,0) 70%); border-radius: 50%; filter: blur(120px); animation: float3 25s infinite ease-in-out;"></div>
        <!-- Removed the heavy white wash to let the colors shine -->
        <div class="hero-glass-overlay" style="position: absolute; inset: 0; background: linear-gradient(180deg, rgba(10,11,16,0.2) 0%, rgba(10,11,16,0.9) 100%);"></div>
    </div>
    ...
    <div id="hero-bg-wa" style="position: absolute; left: -120px; top: 15%; transform: translateY(-50%) rotate(-8deg); z-index: 1; opacity: 0; filter: blur(2px); transition: filter 0.5s;" onmouseover="this.style.filter='blur(0)'" onmouseout="this.style.filter='blur(2px)'">
        <div class="phone-mockup" style="transform: scale(0.65); box-shadow: -20px 20px 60px rgba(0,0,0,0.5);">
    ...
    <div style="position: relative; z-index: 10; max-width: 1000px; display: flex; flex-direction: column; align-items: center; width: 100%;">
      <span class="kicker gsap-fade-up" style="font-family: 'Geist Mono', monospace; font-size: 11px; font-weight: 500; color: #4af; letter-spacing: .24em; text-transform: uppercase; margin-bottom: 12px; display:block;">AI AGENT FOR SALES & SUPPORT</span>
      
      <h1 class="gsap-fade-up" style="font-family: 'Outfit'; font-size: clamp(40px, 6vw, 72px); margin-bottom: 16px; line-height: 1.05; letter-spacing: -0.03em; color: #fff; font-weight: 700; white-space: nowrap;">It answers. Then it acts.</h1>
      
      <p class="gsap-fade-up" style="font-size: clamp(16px, 1.8vw, 18px); color: rgba(255,255,255,0.8); margin-bottom: 32px; max-width: 600px; line-height: 1.5;">StepsAI answers customer questions on WhatsApp, Instagram and your website. Then it books the meeting, saves the lead, or updates the order.</p>
      ...
      <div class="hero-actions gsap-fade-up" style="display:flex; gap:12px; flex-wrap: wrap; justify-content: center; align-items: center;">
        <a href="./partners/apply/index.html" style="text-decoration: none;">
           <button class="btn-primary" style="padding: 14px 28px; font-size: 16px; border-radius: 100px; display: flex; align-items: center; gap: 8px; box-shadow: 0 8px 32px rgba(26,86,219,0.5);">
              Start Free Trial 
        ...
        <button class="btn-outline" style="padding: 14px 28px; font-size: 16px; border-radius: 100px; background: rgba(255,255,255,0.1); color: #fff; border: 1px solid rgba(255,255,255,0.2); backdrop-filter: blur(10px);">Book an Appointment</button>
      </div>
      
      <div class="gsap-fade-up" style="margin-top: 16px; font-size: 12px; color: rgba(255,255,255,0.6); display: flex; align-items: center; gap: 12px; font-weight: 500;">
    ...
    <div id="hero-main-web" class="gsap-fade-up" style="position: relative; z-index: 15; margin-top: 32px; width: 100%; max-width: 380px; opacity: 0; transform: translateY(100px);">
        ...
        <div style="width: 100%; background: #fbfbfd; border-radius: 20px 20px 0 0; box-shadow: 0 40px 100px rgba(0,0,0,0.5), 0 0 0 1px rgba(255,255,255,0.1) inset, 0 10px 30px rgba(123,44,191,0.3); border: 1px solid rgba(255,255,255,0.1); border-bottom: none; overflow: hidden; display: flex; flex-direction: column; height: 400px; position: relative;">
  ...
  <script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        setTimeout(() => {
            const webWidget = document.getElementById('hero-main-web');
            if(webWidget) observer.observe(webWidget);
        }, 2000);
    });
  </script>
\"\"\"
...
if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + CUSTOM_HOME_HERO_V9 + "\n" + content[end_idx:]
    with open('build_v11_compiler.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Injected V9 Centered Hero with BOLD Gradient Background")
else:
